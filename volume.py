def calc_volume(input_dem, pts=None, pts_epsg=None, geojson_polygon=None, decimals=4,
                base_method="triangulate", custom_base_z=363, isAbsolute: bool = False):
    try:
        import os
        import rasterio
        ...
        with rasterio.open(input_dem) as d:
            ...
            vol: float = np.abs(np.round(volume, decimals=decimals)) if isAbsolute else np.round(volume, decimals=decimals)
